...
const WalletConnect: React.FC<WalletConnectProps> = ({ 
  ...
  const { startSync, stopSync, isSyncing, progress } = useSupabaseSyncUI();
  ...
  const handleSync = () => {
    startSync(true); // Force refresh = true (clear all relationships and pull fresh)
  };

  // Label of the sync step currently running, e.g. "agency players"
  const currentSyncStep = progress.find(p => p.status === 'in_progress')?.dataType.split(':')[0].replace(/_/g, ' ');
  ...
  if (isConnected && account) {
    return (
      <div className="relative">
        ...
        >
          {isSyncing ? (
            <svg className="animate-spin w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" role="img" aria-label="Syncing data">
        ...
        {showDropdown && (
          <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50">
            ...
            <div className="p-1">
              {isSyncing ? (
                <>
                  <div className="px-3 py-2 text-sm text-blue-600 dark:text-blue-400 flex items-center space-x-2">
                    <svg className="animate-spin w-4 h-4 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span className="truncate">{currentSyncStep ? `Syncing ${currentSyncStep}...` : 'Syncing...'}</span>
                  </div>
                  <button
                    onClick={stopSync}
                ...
                <button
                  onClick={handleSync}
                  className="w-full text-left px-3 py-2 text-sm text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md transition-colors flex items-center space-x-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                  <span>Sync Agency Data</span>
                </button>
              )}
              <button
