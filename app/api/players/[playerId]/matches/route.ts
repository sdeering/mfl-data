...
export async function GET(
  ...
  try {
    ...
    console.log(`[API] GET /api/players/${playerId}/matches - limit: ${limit}`);
    
    const url = `${MFL_API_BASE_URL}/players/${playerId}/matches/stats?limit=${limit}`;
    console.log(`[API] Fetching from MFL API: ${url}`);

    const controller = new AbortController();
    const timeoutId = setTimeout(() => {
      console.log(`[API] Timeout reached, aborting request for player matches`);
      controller.abort();
    }, TIMEOUT_MS);
    ...
    try {
      ...
      console.log(`[API] Successfully fetched ${Array.isArray(data) ? data.length : 0} match stats`);
      ...
      console.error(`[API] Error fetching player matches (took ${duration}ms):`, error);
    ...
    const duration = Date.now() - startTime;
    console.error(`[API] Unexpected error in player matches proxy (took ${duration}ms):`, error);
