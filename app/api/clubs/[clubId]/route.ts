...
export async function GET(
  ...
  try {
    ...
    console.log(`[API] GET /api/clubs/${clubId}`);
    
    const url = `${MFL_API_BASE}/clubs/${clubId}`;
    console.log(`[API] Fetching from MFL API: ${url}`);
    
    const controller = new AbortController();
    const timeoutId = setTimeout(() => {
      console.log(`[API] Timeout reached, aborting request for club details`);
      controller.abort();
    }, TIMEOUT_MS);
    ...
    try {
      ...
      console.log(`[API] Successfully fetched club details for ${clubId}`);
      ...
      console.error(`[API] Error fetching club details (took ${duration}ms):`, error);
    ...
    const duration = Date.now() - startTime;
    console.error(`[API] Unexpected error in club details proxy (took ${duration}ms):`, error);
