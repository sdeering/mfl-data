...
async function main() {
  console.log('Starting comprehensive player data scraping...');
  ...
  try {
    ...
    // Process each new player (limit to 50 for now)
    for (let i = 0; i < Math.min(newPlayerIds.length, 50); i++) {
      const playerId = newPlayerIds[i];
      console.log(`Processing player ${i + 1}/${Math.min(newPlayerIds.length, 50)}: ${playerId}`);
    ...
    // Save to file
    XLSX.writeFile(workbook, 'Data/comprehensive-player-data.xlsx');
    
    console.log(`Excel file created successfully: ${path.resolve('Data/comprehensive-player-data.xlsx')}`);
